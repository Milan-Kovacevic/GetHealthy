...
CREATE TABLE IF NOT EXISTS `get_healthy`.`TRAINEE_EXERCISING` (
  ...
  `UserId` INT NOT NULL,
  `TrainingProgramOnSchduleId` INT,
  PRIMARY KEY (`Id`),
  ...
  INDEX `fk_TRAINEE_EXERCISING_TRAINING_PROGRAM_ON_SCHEDULE_idx` (`TrainingProgramOnScheduleId` ASC) VISIBLE,
  ...
  CONSTRAINT `fk_TRAINEE_EXERCISING_TRAINEE1`
    ...
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
    CONSTRAINT `fk_TRAINEE_EXERCISING_TRAINING_PROGRAM_ON_SCHEDULE`
    FOREIGN KEY (`TrainingProgramONScheduleId`)
    REFERENCES `get_healthy`.`TRAINING_PROGRAM_ON_SCHEDULE` (`Id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
