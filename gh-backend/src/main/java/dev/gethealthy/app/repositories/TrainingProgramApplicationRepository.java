...
public interface TrainingProgramApplicationRepository extends JpaRepository<TrainingProgramApplication, TrainingProgramApplicationId> {
    Page<TrainingProgramApplication> findByProgram_User_IdOrderByMarkReadAsc(Integer trainerId, Pageable page);
    ...
    @Query("SELECT e from TrainingProgramApplication e WHERE e.program.user.id=:userId " +
    ...
    @Query("SELECT COUNT(e.id) from TrainingProgramApplication e WHERE e.program.user.id = ?1 and e.markRead=false")
